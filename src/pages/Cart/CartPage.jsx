...
import { ShoppingCart, Trash2, Truck, ChevronDown, ChevronUp, Image as ImageIcon } from 'lucide-react';
...
export const CartPage = () => {
  ...
  // Calculate numbers for summary block
  const itemCount = cartCount || 0;
  const rawSubtotal = cartTotal || 0;
  const discountAmount = rawSubtotal > 0 ? 5000 : 0;
  const bonusEarned = Math.round(rawSubtotal * 0.05);
  const bonusSpent = useBonuses && rawSubtotal > 0 ? 5000 : 0;
        ...
        {isEmpty ? (
          ...
          <div className="empty-cart-card">
            ...
            <p className="empty-cart-subtitle">
              Добавляйте товары в избранном,{"\n"}что бы не потерять их и купить позже
            </p>
              ...
              {cart.map(({ product, qty }) => {
                ...
                return (
                  <div className="cart-item-card" key={product.id}>
                    ...
                    <Link to={`/product/${product.id}`} className="cart-item-image-box">
                      {product.image ? (
                        <img
                          src={product.image}
                          alt={product.title}
                    ...
                    <div className="cart-item-body">
                      ...
                      <div className="cart-item-desc-wrapper">
                        ...
                        {isExpanded && (
                          <p className="cart-item-desc-text">
                            {product.description ||
                              "Lorem ipsum dolor sit amet consectetur. Pulvinar imperdiet cum condimentum metus phasellus tortor. Lectus vulputate malesuada sit in. Facilisis amet nulla urna pulvinar iac..."}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="cart-item-delivery">
                        <Truck size={15} />
                        <span>Доставка: <span className="delivery-dates">25–27 мая</span></span>
                      </div>
                      ...
                      <div className="cart-item-bottom">
                        ...
                        <button
                          className="btn-buy-card btn-primary-red"
                          onClick={() => showToast(`Оформляем "${product.title}"`)}
                          type="button"
                        >
                          Купить
                        </button>
