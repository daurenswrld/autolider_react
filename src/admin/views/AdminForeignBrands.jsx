...
export const AdminForeignBrands = () => {
  const { showToast, refreshForeignBrands } = useApp();
  ...
  const fetchForeignBrands = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch foreign brands:", err);
      showToast("Ошибка загрузки иномарок", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveBrand = async (e) => {
    ...
    if (!brandForm.name.trim()) {
      showToast("Укажите название иномарки", "error");
      return;
    }

    try {
      const url = editingBrand
        ? `/api/foreign-brands/${editingBrand.id}`
        : "/api/foreign-brands";
      ...
      if (res.ok) {
        showToast(
          editingBrand
            ? "Иномарка обновлена!"
            : "Новая иномарка успешно добавлена!",
        );
        setIsBrandModalOpen(false);
        fetchForeignBrands();
        if (refreshForeignBrands) refreshForeignBrands();
      } else {
        const errData = await res.json();
        showToast(errData.message || "Ошибка сохранения", "error");
      }
    } catch (err) {
      console.error("Failed to save foreign brand:", err);
  ...
  const handleDeleteBrand = async (brandId, brandName) => {
    if (
      !window.confirm(`Удалить иномарку "${brandName}" и все её модели?`)
    )
      return;
    ...
    try {
      ...
      if (res.ok) {
        showToast(`Иномарка "${brandName}" удалена!`);
        fetchForeignBrands();
        if (refreshForeignBrands) refreshForeignBrands();
      }
    } catch (err) {
      console.error("Failed to delete foreign brand:", err);
    }
  };
  ...
  const handleSaveModel = async (e) => {
    ...
    try {
      const url = editingModel
        ? `/api/foreign-brands/${activeBrandForModel.id}/models/${editingModel.id}`
  ...
  return (
    <div className="admin-view-container">
      ...
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">Иномарки на главной</h1>
          <p className="admin-page-subtitle">
            Управление иномарками в левом сайдбаре главной страницы и их моделями
          </p>
        </div>
        <div className="admin-header-actions">
          <button
            className="btn-admin-secondary"
            onClick={fetchForeignBrands}
            title="Обновить"
          >
      ...
      >
        ...
        >
          ...
          <input
            type="text"
            placeholder="Поиск по названию иномарки..."
            value={searchTerm}
      ...
      {loading ? (
        <div className="admin-card loading-box">
          ...
          <span>Загрузка иномарок и моделей...</span>
        </div>
      ) : filteredBrands.length === 0 ? (
        <div className="admin-card empty-brands-box">
          <Car size={48} />
          <h3>Иномарки не найдены</h3>
          <p>Добавьте первую марку авто с логотипом</p>
          <button
            className="btn-admin-primary"
            onClick={() => handleOpenBrandModal()}
          >
                  ...
                  >
                    ...
                    {brand.status === "disabled" && (
                      <EyeOff
                        size={18}
                        color="#ef4444"
                        title="Марка отключена (скрыта на сайте)"
                        style={{ flexShrink: 0 }}
                      />
                    )}
                  </div>
          ...
          <div className="admin-modal-box">
            ...
            <form onSubmit={handleSaveBrand} className="admin-form">
              <div className="form-group">
                <label>
                  Название марки (например: MERCEDES, TOYOTA, BMW) *
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="form-group">
                ...
                >
                  <option value="enabled">
                    Активна (отображается на главной)
                  </option>
                  <option value="disabled">
                    Отключена (скрыта с сайта)
                  </option>
                </select>
              </div>
        ...
        <div className="admin-modal-backdrop">
          <div className="admin-modal-box">
            ...
            <form onSubmit={handleSaveModel} className="admin-form">
              <div className="form-group">
                <label>Название модели (например: Camry XV70, E-Class, X5) *</label>
                <input
                  type="text"
                  required
